...
try:
    ...
    # --- HTML INTRO BLOCK (only shown if data loaded successfully) ---
    st.markdown(
        """
        ...
        with st.container(border=True):
            ...
            if city_data.empty:
                ...
                if plot_data.empty:
                    ...
                    # Color logic - use only categories that exist in data
                    sorted_data["afford_label"] = sorted_data["affordability_rating"].astype(str)
                    unique_cats = sorted_data["afford_label"].unique().tolist()
                    
                    # Build ordered categories list - only include categories in data
                    ordered_categories = []
                    for cat in AFFORDABILITY_CATEGORIES.keys():
                        if cat in unique_cats:
                            ordered_categories.append(cat)
                    if 'N/A' in unique_cats:
                        ordered_categories.append('N/A')
                    
                    # Only use categories that exist in data
                    ...
                    # Create color map only for categories in data
                    filtered_color_map = {cat: AFFORDABILITY_COLORS[cat] for cat in unique_cats if cat in AFFORDABILITY_COLORS}
                    if 'N/A' in unique_cats:
                        filtered_color_map['N/A'] = "#808080"
